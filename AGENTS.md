...
- Calendar lookup: phrase → `CALENDAR_RE` → webhook command `calendar` (MacroDroid Webhook trigger); MacroDroid reopens E.V as `https://elben08.github.io/E.V/?next=<event>` (query string so MacroDroid's "URL encode parameters" encodes the title); `parseCalendarFragment()`/`init()` read it (also supports legacy `#next=` hash), show it, store it as `{ role:'user', text:'[Calendar lookup result] …', sensitive: true }`, then clear the query/hash via `history.replaceState`. MacroDroid config (5.65+ specifics) is in README and MACRODROID.md.
